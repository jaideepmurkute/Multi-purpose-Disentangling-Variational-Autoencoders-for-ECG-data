...
def kl_divergence(a, b, prior_alpha = 5., prior_beta = 1., log_beta_prior = np.log(1./5.), num_terms=10, args=Namespace()):
    ...
    first_term = ((a - prior_alpha)/(a+SMALL)) * (-1 * EULER_GAMMA - torch.digamma(b.view(-1, 1)).view(b.size()) - 1./(b+SMALL))
    ...
    #else:
    sum_term = Variable(torch.DoubleTensor(a.size()).to(args.device).zero_())

    # we should figure out if this is enough
    for i in range(1, num_terms+1):
        # beta_ = Beta()
        # sum_term += beta_(float(i)/(a.view(-1, 1) + SMALL), b.view(-1, 1)).view(a.size())/(i + a * b)
        concentration_1 = float(i)/(a.view(-1, 1) + SMALL)
        concentration_2 = b.view(-1, 1)
        sum_term += torch.distributions.beta.Beta(concentration_1, concentration_2).sample().view(a.size())  / (i + a * b)
